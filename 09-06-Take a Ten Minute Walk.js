...
to walk (eg. ['n', 's', 'w', 'e']). You know it takes you one minute to traverse one city block,
so create a function that will return true if the walk the app gives you will take you exactly ten
minutes (you don't want to be early or late!) and will return you to your starting point.
...
const isValidWalk = (walk) => {
  const countSteps = direction => walk.filter(e => e === direction).length;
  return countSteps('n') === countSteps('s') && countSteps('e') === countSteps('w') && walk.length === 10;
};